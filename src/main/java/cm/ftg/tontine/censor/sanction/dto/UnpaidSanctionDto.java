...
public record UnpaidSanctionDto(
        UUID id,
        UUID tontineId,
        UUID memberId,
    ...
    public static UnpaidSanctionDto from(Sanction s, Instant now) {
        long days = (s.getIssuedAt() == null) ? 0 : Duration.between(s.getIssuedAt(), now).toDays();
        return new UnpaidSanctionDto(
